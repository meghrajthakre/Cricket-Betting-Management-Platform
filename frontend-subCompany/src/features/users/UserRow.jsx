...
export default function UserRow({ user, index, busy, onToggle, onChangePassword, onDelete, onEditCoins, onLimitUpdated, showToast, mode = "clients" }) {
  return (
    <tr className="border-b border-gray-100 transition hover:bg-blue-50/30">
      ...
      {mode === "clients" && <><td className="border border-gray-200 px-5 py-3 text-right font-bold tabular-nums">{Number(user.fixLimit || 0).toFixed(2)}</td><td className="border border-gray-200 px-5 py-3 text-right font-semibold text-blue-700 tabular-nums">{Number(user.currentLimit ?? user.fixLimit ?? user.coins ?? 0).toFixed(2)}</td></>}
