...
export default function EditCoinsModal({
  isOpen,
  user,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateUserBalance(user._id, newValue);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
        ...
        <div className="mb-4">
          ...
          {hasDiff && (
            <p
              className={`text-xs mt-1.5 font-medium ${diff > 0 ? "text-green-600" : "text-red-500"}`}
            >
              {diff > 0 ? `+${diff}` : diff} coins will be{" "}
              {diff > 0 ? "credited" : "debited"}
            </p>
          )}
        </div>
