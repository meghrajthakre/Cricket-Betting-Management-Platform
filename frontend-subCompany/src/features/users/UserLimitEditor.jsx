...
export default function UserLimitEditor({ user, busy, onUpdated, showToast }) {
  ...
  const updateLimit = async () => {
    ...
    if (currentValue < Number(user.pendingUsedLimit || 0)) return showToast(`Current Limit pending used limit ${user.pendingUsedLimit || 0} se kam nahi ho sakti`, true);
  ...
  const usedLimitColor = Number(user.pendingUsedLimit || 0) > 0
    ? "text-amber-600"
    : usedLimit < 0
      ? "text-emerald-600"
      : usedLimit > 0
  ...
  return <>
    ...
    <td className="border border-gray-200 px-3 py-2"><input type="text" value={usedLimit.toFixed(2)} readOnly className={`${inputClass} cursor-default bg-slate-50 font-bold ${usedLimitColor}`} aria-label={`Used limit for ${user.username}`} /></td>
