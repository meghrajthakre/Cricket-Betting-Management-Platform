...
const getUser = (bet) => {
  const populated =
    bet.userId && typeof bet.userId === "object" ? bet.userId : null;
  return {
        ...
        <div className="overflow-y-auto p-4 sm:p-6">
          {loading && (
            <div className="grid min-h-52 place-items-center">
              <Spinner size={34} variant="ocean" label="Bet slips loading" />
            </div>
          )}
          {!loading && error && (
          ...
          {!loading && !error && bets.length === 0 && (
            <div className="grid min-h-52 place-items-center rounded-xl border border-dashed border-(--color-border) bg-(--color-bg-main) text-sm text-slate-500">
              No bet slips found.
            </div>
          )}
            ...
            <div className="grid gap-3">
              {bets.map((bet) => {
                ...
                return (
                  <article
                    key={bet._id}
                    className="grid gap-4 rounded-xl border border-(--color-border) bg-white p-4 transition hover:border-(--color-accent) sm:grid-cols-[1.2fr_1fr_auto] sm:items-center"
                  >
                    ...
                    <div>
                      <p className="text-sm font-bold text-(--color-text-dark)">
                        {bet.selectionName || bet.marketId || "Selection"}
                      </p>
                    ...
                    <div className="flex items-center justify-between gap-3 sm:justify-end">
                      <span
                        className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase ${pending ? "bg-amber-50 text-amber-700" : bet.status === "won" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}
                      >
                        {bet.status}
                      </span>
                      {!readOnly && (
                        <button
                          ...
                          disabled={deletingId === bet._id}
                          title="Delete bet slip"
                          className="grid h-9 w-9 cursor-pointer place-items-center rounded-lg border border-red-200 bg-red-50 text-red-500 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          {deletingId === bet._id ? (
                            <Spinner
                              size={16}
...
export default function BetSlipsPanel(props) {
  ...
  return (
    <BetSlipPanel
      ...
      bets={bets}
      title="Bet Slips"
      subtitle="Match market bets placed by users"
    />
  );
}
