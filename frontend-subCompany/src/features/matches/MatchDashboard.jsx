...
import { fetchMatchBets } from "./live-report/api/liveReportApi";
  ...
  return (
    <section className="min-h-full text-(--color-text-dark)">
      <header className="mb-7">
        <h1 className="text-[30px] font-normal leading-tight text-(--color-text-dark)">
          Matches
        </h1>
        <nav className="mt-3 flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="text-gray-300">/</span>
          <span
            className="max-w-[75vw] truncate font-bold text-(--color-text-dark)"
            title={title}
          >
            {title}
          </span>
            ...
            <div className="flex flex-wrap justify-center gap-1 px-5 py-5">
              {reportButtons.map((button) =>
                button.path ? (
                  <Link
                    key={button.label}
                    to={`/sub-company/matches/${encodeURIComponent(id)}/${button.path}`}
              ...
              <table className="w-full min-w-[650px] border-collapse text-left text-sm">
                ...
                <tbody>
                  {marketRows.map((market) => (
                    <tr
                      key={market.marketId}
                      className="hover:bg-(--color-bg-main)"
                    >
