...
export default function SearchBar({ search, onSearchChange, onSearch, onClear, loading, onCreate }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-(--color-border) p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
      ...
      <button type="button" onClick={onCreate} className="flex min-h-10 items-center justify-center gap-2 rounded-xl bg-(--color-btn-bg) px-4 py-2.5 text-sm font-bold text-white hover:bg-(--color-btn-hover)"><Plus size={17} />Create User</button>
    </div>
  );
}
