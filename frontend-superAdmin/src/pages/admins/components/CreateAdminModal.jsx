...
export default function CreateAdminModal({ open, availableShare, onClose, onCreated, showToast }) {
  ...
  return (
    <Modal open={open} title="Create Admin" onClose={saving ? () => {} : onClose}>
      ...
      <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} disabled={saving} className="rounded-xl border border-(--color-border) px-4 py-2.5 text-sm font-semibold text-gray-600 disabled:opacity-50">Cancel</button><button type="button" onClick={submit} disabled={saving || adminShare === "Invalid"} className="flex items-center gap-2 rounded-xl bg-(--color-btn-bg) px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? <Spinner size={16} variant="neon" /> : <Plus size={16} />}{saving ? "Creating..." : "Create Admin"}</button></div>
    </Modal>
  );
}
